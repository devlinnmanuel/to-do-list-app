<script>
    ...
    methods: {
      ...
      async markAsDone(taskId) {
          try {
            await axios.put(`http://localhost:5000/api/${taskId}/done`);
            this.editingTaskId = null;
            this.showForm1 = false;
      ...
      toggleAll() {
        if (this.selectAll) {
          ...
          this.selectedKategoriFilters = ['Important', 'Urgent', 'Reguler', 'Done'];
        } else {
    ...
    <div class="background-web">
      ...
      <div class="toolbar">
        ...
        <div class="filter-container">
          ...
          <label id="done-task">
            <input type="checkbox" value="Done" v-model="selectedKategoriFilters" />
            Done
          </label>
          <label id="all-task">
...
.important {
  ...
  max-width: 100px; 
  word-wrap: break-word; /* Paksa kata panjang buat baris baru */
  overflow-wrap: break-word;
  white-space: normal; /* teks untuk membungkus */
  border-radius: 4%;
  padding: 20px;
}
...
@media (max-width: 768px) {
  ...
  /* tombol add task yang floating di mobile: */
  #add-task {
    position: fixed;
    bottom: 20px;
    right: 20px; 
